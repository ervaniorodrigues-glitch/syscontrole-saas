...
SysControleWeb.prototype.toggleCert35Page = function(delta) {
    this.cert35_page = (this.cert35_page === 0) ? 1 : 0;
    this.renderCertNR35();
};

SysControleWeb.prototype.abrirCertNR35 = async function() {
    if (!this.editingId) {
    ...
    const dataEmi = document.getElementById('nr35_dataEmissao')?.value;
    if (!dataEmi) {
        if (typeof this.showToast === 'function') {
    ...
    // Carregar configuração da NR-35 (Tenta servidor, cai no localStorage)
    let cfg = JSON.parse(localStorage.getItem('cfg_nr35') || '{}');
    try {
        const response = await fetch('/api/config/nr/nr35');
        const resJson = await response.json();
        if (resJson.success && resJson.dados) {
            cfg = resJson.dados;
            localStorage.setItem('cfg_nr35', JSON.stringify(cfg));
        }
    } catch (e) { console.error('Erro ao buscar config NR35 do servidor:', e); }

    document.getElementById('cfg35_instrutor').value       = cfg.instrutor || '';
    document.getElementById('cfg35_cargo_instrutor').value = cfg.cargo_instrutor || '';
    document.getElementById('cfg35_reg_instrutor').value   = cfg.reg_instrutor || '';
    document.getElementById('cfg35_responsavel').value     = cfg.responsavel || '';
    document.getElementById('cfg35_cargo_resp').value      = cfg.cargo_resp || '';
    document.getElementById('cfg35_reg_resp').value        = cfg.reg_resp || '';
    document.getElementById('cfg35_local').value           = cfg.local || '';
    document.getElementById('cfg35_uf').value              = cfg.uf || 'SP';
    const logoInput = document.getElementById('cfg35_logo');
    const logoPreview = document.getElementById('cfg35_logo_preview');
    if (logoInput) logoInput.value = cfg.logo || '';
    if (logoPreview) logoPreview.src = cfg.logo || '/Logo-Hoss.jpg';
    document.getElementById('cert35_num').value            = this.currentEditingData?.Nr35_NumControle || '';
    
    const modal = document.getElementById('modalCertNR35');
    if (modal) modal.style.display = 'block';
    
    this.cert35_page = 0; // Reset to front page
    this.renderCertNR35();
};

SysControleWeb.prototype.abrirCfgNR35 = async function() {
    // Carregar configuração da NR-35 (Tenta servidor, cai no localStorage)
    let cfg = JSON.parse(localStorage.getItem('cfg_nr35') || '{}');
    try {
        const response = await fetch('/api/config/nr/nr35');
    ...
    this.cfg35_anos = cfg.anos || [new Date().getFullYear()];
    this.cfg35_ano_selecionado = cfg.ano_selecionado || new Date().getFullYear();
    this.atualizarListaAnos35();

    const modal = document.getElementById('modalCfgNR35');
    if (modal) modal.style.display = 'block';
    
    // Garantir que o certificado esteja aberto/visível por baixo
    const modalCert = document.getElementById('modalCertNR35');
    if (modalCert && modalCert.style.display === 'none') {
        this.renderCertNR35();
    }
};

SysControleWeb.prototype.adicionarAno35 = function() {
    const input = document.getElementById('cfg35_novo_ano');
    ...
    if (this.cfg35_anos.includes(ano)) {
        if (typeof this.showToast === 'function') this.showToast('Ano já existe', 'warning');
        return;
    }
    this.cfg35_anos.push(ano);
    this.cfg35_anos.sort((a,b) => b - a);
    this.cfg35_ano_selecionado = ano;
    input.value = '';
    this.atualizarListaAnos35();
};

SysControleWeb.prototype.removerAnoSelecionado35 = function() {
    const select = document.getElementById('cfg35_ano_ativo');
    const ano = parseInt(select.value);
    if (!ano) return;
    if (confirm(`Tem certeza que deseja excluir o ano ${ano} da lista?`)) {
        this.removerAno35(ano);
    }
};

SysControleWeb.prototype.removerAno35 = function(ano) {
    const anoNum = Number(ano);
    this.cfg35_anos = this.cfg35_anos.filter(a => Number(a) !== anoNum);
    if (Number(this.cfg35_ano_selecionado) === anoNum) {
        this.cfg35_ano_selecionado = this.cfg35_anos[0] || new Date().getFullYear();
    }
    this.atualizarListaAnos35();
    if (typeof this.showToast === 'function') this.showToast('Ano removido da lista!', 'info');
};

SysControleWeb.prototype.atualizarListaAnos35 = function() {
    const select = document.getElementById('cfg35_ano_ativo');
    if (!select) return;
    select.innerHTML = '';
    this.cfg35_anos.forEach(ano => {
        const opt = document.createElement('option');
        opt.value = ano;
        opt.textContent = ano;
        if (parseInt(ano) === parseInt(this.cfg35_ano_selecionado)) opt.selected = true;
        select.appendChild(opt);
    });
};

SysControleWeb.prototype.salvarCfgNR35 = async function() {
    const anoAtivo = parseInt(document.getElementById('cfg35_ano_ativo').value);
    const cfgData = {
        instrutor:       document.getElementById('cfg35_instrutor').value,
        cargo_instrutor: document.getElementById('cfg35_cargo_instrutor').value,
        reg_instrutor:   document.getElementById('cfg35_reg_instrutor').value,
        responsavel:     document.getElementById('cfg35_responsavel').value,
        cargo_resp:      document.getElementById('cfg35_cargo_resp').value,
        reg_resp:        document.getElementById('cfg35_reg_resp').value,
        local:           document.getElementById('cfg35_local').value,
        uf:              document.getElementById('cfg35_uf').value,
        logo:            document.getElementById('cfg35_logo').value,
        anos:            this.cfg35_anos,
        ano_selecionado: anoAtivo
    };

    // Salva no localStorage para velocidade
    localStorage.setItem('cfg_nr35', JSON.stringify(cfgData));
    
    // Salva no SERVIDOR para persistência e backup (Crítico)
    try {
        await fetch('/api/config/nr/nr35', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ dados: cfgData })
        });
    } catch (e) {
        console.error('Erro ao salvar config NR35 no servidor:', e);
    }
    
    // Sincronizar ano com o registro atual se estiver editando
    if (this.editingId && this.currentEditingData) {
        this.currentEditingData.Nr35_AnoControle = anoAtivo;
        const inputNum = document.getElementById('cert35_num');
        // Agora salva mesmo que o número esteja vazio para o ano carregar corretamente
        const success = await this.verificarESalvarNumeroControle('nr35', (inputNum?.value || ''), inputNum, false);
...
SysControleWeb.prototype.fecharCfgNR35 = function() {
    const modal = document.getElementById('modalCfgNR35');
    if (modal) modal.style.display = 'none';
};

SysControleWeb.prototype.processarLogoUploadNR35 = function(input) {
...
SysControleWeb.prototype.renderCertNR35 = function() {
    ...
    const numero = document.getElementById('cert35_num')?.value || this.currentEditingData?.Nr35_NumControle || '___';
    ...
    if (numero !== '___' && !this.currentEditingData?.Nr35_NumControle) {
        localStorage.setItem('cert35_ultimo_num', numero);
    }
    const dataEmissao = document.getElementById('nr35_dataEmissao')?.value || '';

    const cfg = JSON.parse(localStorage.getItem('cfg_nr35') || '{}');
    const instrutor      = this._corrigirTexto(cfg.instrutor      || '_______________________');
    const cargoInstrutor = this._corrigirTexto(cfg.cargo_instrutor || 'Instrutor de Trabalho em Altura');
    ...
    const numManualNum = document.getElementById('cert35_num')?.value || this.currentEditingData?.Nr35_NumControle || '';
    const ano = (numManualNum ? (this.currentEditingData?.Nr35_AnoControle || cfg.ano_selecionado) : cfg.ano_selecionado) || hoje.getFullYear();
    const dataExibir = dataEmissao || (dia+'/'+mes+'/'+ano);


    const currentPage = this.cert35_page || 0;

    const preview = document.getElementById('certNR35_preview');
    if (!preview) return;
    ...
    const isInvalido = this.isCertificadoInvalido(liveData, 'nr35');
    const isVencido = this.isCertificadoVencido(liveData, 'nr35');
    let watermarkHTML = '';
    ...
    <style id="cert35-style">
        ...
        @media print {
            ...
            #certNR35_preview, #certNR35_preview * { visibility: visible !important; }
            #certNR35_preview { position: absolute !important; top: 0 !important; left: 0 !important; width: 297mm !important; margin: 0 !important; padding: 0 !important; background: #fff !important; display: block !important; overflow: visible !important; }
            @page { margin: 0; }
    ...
    <div class="cert-nav-arrow left" onclick="syscontrole.toggleCert35Page(-1)">&lsaquo;</div>
    <div class="cert-nav-arrow right" onclick="syscontrole.toggleCert35Page(1)">&rsaquo;</div>
    ...
    <div class="cert-scale-wrapper page-0">
        <div class="cert-container cert-frente">
            ...
            <div class="cert-inner-content">
                <div class="controle-texto-original">Controle nº NR 35 - ${ano} - ${numero}</div>
                ...
                <div class="cert-body">
                    ...
                    frequentou e concluiu satisfatoriamente o <strong>Treinamento de Trabalho em Altura</strong>,
                    com dura&ccedil;&atilde;o de <strong>8 horas</strong>, do Minist&eacute;rio
                    do Trabalho, realizado no per&iacute;odo de 08:00 as 17:00 horas, em atendimento &agrave; <strong>NR &ndash; 35 Portaria SIT n.&ordm; 313, de 23 de mar&ccedil;o de 2012 da Portaria N&ordm; 3214/78</strong>.
    ...
    <div class="cert-scale-wrapper page-1">
        <div class="cert-container cert-verso">
            ...
            <div class="corner-ornament bottom-right"><div class="corner-circle"></div></div>
            <div class="cert-frame-rectangle"></div>
            <div class="cert-inner-content" style="padding: 25mm 25mm 10mm 25mm; display: flex; flex-direction: column; justify-content: flex-start;">
                <div class="controle-texto-original">Controle nº NR 35 - ${ano} - ${numero}</div>
                ...
                <div style="display: flex; align-items: center; width: 100%; margin-bottom: 5mm; padding: 0 5mm;">
                    ...
                    <div style="flex: 1; text-align: center; font-family: 'Lato', sans-serif; color: #1a3a6b;">
                        <div style="font-size: 24pt; font-weight: bold;">CONTE&Oacute;DO PROGRAM&Aacute;TICO</div>
                        <div style="font-size: 13pt; font-weight: bold; margin-top: 1.5mm;">
                            NR &ndash; 35 Portaria SIT n.&ordm; 313, de 23 de mar&ccedil;o de 2012
                        </div>
                    </div>
                    <div style="width: 50mm;"></div> <!-- Balanceador para centralização perfeita -->
                </div>

                <div style="font-size: 16pt; line-height: 1.6; color: #222; margin-left: 17mm; font-family: 'Lato', sans-serif;">
                    1. Normas e Regulamentos aplicáveis ao trabalho em altura: NR35 Aplicada e comentada.<br>
                    2. Análise de Riscos.<br>
                    3. Riscos potenciais inerentes ao trabalho em altura e medidas de prevenção e controle.<br>
                    4. Equipamentos de Proteção Individual e Coletiva para Trabalho em Altura.<br>
                    5. Acidentes Típicos em Trabalho em Altura.<br>
                    6. Condutas em Situações de Emergência, incluindo noções Técnicas de Resgate e Primeiros Socorros.<br>
                    7. Quedas.<br>
                    8. Forças geradas por quedas.<br>
                    9. Força de impacto, absorção de choque, fator de quedas.<br>
                    10. Sistema de ancoragem.<br>
                    11. Cordas nós, cabo guia, linha de vida, andaimes.<br>
                    12. Trabalhos em telhado.<br>
                    13. Risco indiretos.<br>
                    14. Treinamento prático.
                </div>
            </div>
        </div>
    </div>`;
    ...
    preview._printHandler = function(e) {
        if (e && e.ctrlKey && e.key.toLowerCase() === 'p' && document.getElementById('modalCertNR35').style.display === 'block') {
            e.preventDefault();
            if(typeof syscontrole !== 'undefined' && syscontrole.imprimirCertificado) syscontrole.imprimirCertificado('nr35', 'ambos');
        }
    };
    window.addEventListener('keydown', preview._printHandler);
};

SysControleWeb.prototype.abrirLPTNR35 = async function() {
    if (!this.editingId) {
    ...
    const modal = document.getElementById('modalLPTNR35');
    if (modal) modal.style.display = 'block';
    
    this.renderLPTNR35();
};

SysControleWeb.prototype.renderLPTNR35 = function() {
    const nome        = this._corrigirTexto((document.getElementById('nome')?.value || '').toUpperCase());
    const funcao      = this._corrigirTexto((document.getElementById('funcao')?.value || '').toUpperCase());
    ...
    const cargoInstrutor = cfg.cargo_instrutor || 'Instrutor de Trabalho em Altura';
    ...
    const preview = document.getElementById('lptNR35_preview');
    if (!preview) return;

    preview.innerHTML = `
    <style id="lpt35-style">
        @import url('https://fonts.googleapis.com/css2?family=Lato:wght@300;400;700&display=swap');

        @media screen {
            #lptNR35_preview { padding: 20px; background: #f0f0f0; display: flex; justify-content: center; overflow-y: auto; }
        ...
        @media print {
            ...
            body * { visibility: hidden !important; }
            #lptNR35_preview, #lptNR35_preview * { visibility: visible !important; }
            #lptNR35_preview { position: absolute; top: 0; left: 0; width: 100%; padding: 0; margin: 0; background: #fff; }
    ...
    <div class="lpt-page">
        <div class="lpt-header">
            ...
            <div class="lpt-title-box">
                <div class="lpt-title-top">TREINAMENTO NR 35<br>TRABALHO EM ALTURA</div>
        ...
        <div class="lpt-topics">
            <ol>
                <li>Normas e Regulamentos aplicáveis ao trabalho em altura: NR35 Aplicada e comentada.</li>
                ...
                <li>Equipamentos de Proteção Individual e Coletiva: seleção, inspeção, conservação e uso.</li>
                <li>Acidentes Típicos em Trabalho em Altura e causas principais.</li>
                <li>Condutas em Situações de Emergência: noções de técnicas de resgate e Primeiros Socorros.</li>
                <li>Treinamento Pr&aacute;tico: Nós e amarrações, ancoragens, sistemas de descida e ascensão.</li>
            </ol>
        </div>
        ...
        <div class="lpt-details-grid">
            ...
            <div class="lpt-detail-item" style="text-align: center;">
                ...
                <span class="lpt-detail-value" style="min-width: 80%; font-weight: bold;">08 HORAS</span>
            </div>
...
SysControleWeb.prototype.fecharLPTNR35 = function() {
    const modal = document.getElementById('modalLPTNR35');
    if (modal) modal.style.display = 'none';
    ...
    // Garantir que volta para a aba NR-35
    if (typeof this.switchNRTab === 'function') {
        this.switchNRTab('nr35');
    }
};

SysControleWeb.prototype.fecharCertNR35 = function() {
    const modal = document.getElementById('modalCertNR35');
    if (modal) modal.style.display = 'none';
