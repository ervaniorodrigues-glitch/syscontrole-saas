...
class SysControleWeb {
    ...
    async abrirControlePresenca() {
        ...
        try {
            ...
            const funcoesUnicas = [...new Set(funcionarios.map(f => f.Funcao ? f.Funcao.toUpperCase().trim() : '').filter(f => f))].sort((a, b) => {
                // Ordenar alfabeticamente normalmente
            ...
            for (const func of funcionarios) {
                ...
                for (let dia = 1; dia <= diasNoMes; dia++) {
                    ...
                    // Só mostra azul se foi marcado manualmente como folga (ponto) OU se é fim de semana
                    const statusClass = status ? `status-${status.toLowerCase()}` : (isFolgaSalva || isFimDeSemana ? 'dia-folga-input' : '');
                    ...
                    bodyHtml += `<input type="text" class="presenca-input ${statusClass} ${comentarioClass} ${classeDiaFora}" `;
                    bodyHtml += `data-func="${func.id}" data-dia="${dia}" data-funcao="${func.Funcao || ''}" data-empresa="${func.Empresa || ''}" data-folga="${isFolgaSalva || isFimDeSemana}" ${comentarioData} ${comentarioTitle} `;
                    bodyHtml += `value="${diaForaPeriodo ? '' : status}" maxlength="2" ${readonlyAttr} `;
                    
                    // TODOS os usuários podem ver tooltips de comentários (exceto dias fora do período)
                    if (!diaForaPeriodo) {
                        bodyHtml += `onmouseover="syscontrole.mostrarTooltipComentario(this)" `;
                        bodyHtml += `onmouseout="syscontrole.esconderTooltipComentario(this)" `;
                    }
                    
                    // Apenas usuários MASTER e funcionários não-inativados e dentro do período podem editar
                    if (!isComum && !isIntermediario && !isInativado && !diaForaPeriodo) {
            ...
            // Ordenar alfabeticamente (HOSS sempre em primeiro)
            empresasAtivas.sort((a, b) => {
                if (a === 'HOSS' && b !== 'HOSS') return -1;
                if (a !== 'HOSS' && b === 'HOSS') return 1;
                return a.localeCompare(b);
            });
            empresasInativas.sort((a, b) => {
                if (a === 'HOSS' && b !== 'HOSS') return -1;
                if (a !== 'HOSS' && b === 'HOSS') return 1;
                return a.localeCompare(b);
            });
    ...
    formatarPresenca(input) {
        ...
        // ============================================
        // REGRA IMUTÁVEL - NÃO ALTERAR SEM AUTORIZAÇÃO DO ADMINISTRADOR
        // ============================================
        // Se digitou ponto ou hífen, marca como folga (azul) e MANTÉM O PONTO
        // Só limpa se o usuário apertar DELETE ou BACKSPACE
        // ============================================
        if (valor === '.' || valor === '-') {
            // NÃO LIMPAR O VALOR! Manter o ponto para ficar azul
            input.value = '.';
        ...
        // ============================================
        // REGRA IMUTÁVEL - MANTER AZUL QUANDO FOLGA
        // ============================================
        // Se tinha folga marcada e está vazio, MANTER a classe azul
        if (!valor && input.dataset.folga === 'true') {
            // NÃO RESETAR A CLASSE! Manter dia-folga-input
            input.className = 'presenca-input dia-folga-input';
        } else {
            // Só reseta a classe se NÃO for folga
            input.className = 'presenca-input';
            
            if (valor) {
                // Remove a folga se digitou algo
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // NÃO registrar entrada aqui - já é feito no verificarLogin()
    // if (window.syscontrole.currentUser) {
    //     window.syscontrole.registrarEntradaSistema();
    // }
