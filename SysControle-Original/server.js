...
async function gerarBackupPresenca(mesAno) {
    try {
        ...
        const funcionarios = await new Promise((resolve, reject) => {
            db.all(`SELECT id, Nome, Empresa, Funcao FROM SSMA WHERE Situacao = 'N'`, [], (err, rows) => {
                if (err) reject(err);
                else {
                    // ⭐ ORDENAÇÃO CUSTOMIZADA: HOSS SEMPRE EM PRIMEIRO
                    rows.sort((a, b) => {
                        // HOSS sempre em primeiro
                        if (a.Empresa === 'HOSS' && b.Empresa !== 'HOSS') return -1;
                        if (a.Empresa !== 'HOSS' && b.Empresa === 'HOSS') return 1;
                        // Se ambas são HOSS, ordenar por nome
                        if (a.Empresa === 'HOSS' && b.Empresa === 'HOSS') return a.Nome.localeCompare(b.Nome);
                        // Outras empresas: ordenar alfabeticamente por empresa, depois por nome
...
function initDatabase() {
    ...
    db.run(`ALTER TABLE SSMA ADD COLUMN Genero TEXT DEFAULT 'M'`, (err) => {
        // Ignora erro se coluna já existe
    });
    ...
    db.get('SELECT COUNT(*) as count FROM configuracao_relatorio', (err, row) => {
        ...
        // ⚠️ IMPORTANTE: Criar índice UNIQUE no CPF (normalizado) para garantir que não haja duplicatas
        // Isso vai bloquear qualquer tentativa de inserir/atualizar com CPF duplicado
        db.run(`
            CREATE UNIQUE INDEX IF NOT EXISTS idx_ssma_cpf_unique 
            ON SSMA(UPPER(TRIM(CPF))) 
            WHERE CPF IS NOT NULL AND CPF != ''
        `, (err) => {
            if (err) {
                console.warn('⚠️ Aviso ao criar índice UNIQUE de CPF:', err.message);
                // Não é erro fatal, apenas aviso
            } else {
                console.log('🔒 Índice UNIQUE no CPF criado com sucesso!');
            }
        });
...
app.get('/api/ssma/check-cpf', (req, res) => {
    ...
    // Normalizar CPF: remover espaços e converter para maiúsculo
    const cpfNormalizado = cpf.trim().toUpperCase();
    console.log('🔍 Verificando CPF duplicado:', cpfNormalizado, 'excludeId:', excludeId);
    
    let sql = `
        SELECT id, Nome, Empresa, Situacao FROM SSMA 
        WHERE UPPER(TRIM(CPF)) = ?
    `;
    const params = [cpfNormalizado];
    ...
    db.get(sql, params, (err, row) => {
        ...
        if (row) {
            ...
            console.log('⚠️ CPF duplicado encontrado:', row, 'Status:', statusText);
            res.json({ 
                exists: true, 
                id: row.id,
                nome: row.Nome,
...
app.get('/api/ssma/:id', (req, res) => {
    ...
    db.get('SELECT * FROM SSMA WHERE id = ?', [id], (err, row) => {
        ...
        // ✅ LIMPEZA DE DADOS ORFÃOS: Se não tem data de emissão, limpar vencimento
        const nrs = ['Nr06', 'Nr10', 'Nr11', 'NR12', 'Nr17', 'NR18', 'Nr20', 'NR33', 'Nr34', 'NR35'];
        nrs.forEach(nr => {
            const dataEmissaoKey = `${nr}_DataEmissao`;
            const vencimentoKey = nr === 'NR12' || nr === 'NR18' || nr === 'NR33' || nr === 'NR35' ? 
                                  `${nr}_Vencimento` : `${nr}_Vencimento`;
            const statusKey = nr === 'NR12' || nr === 'NR18' || nr === 'NR33' || nr === 'NR35' ? 
                              `${nr}_Status` : `${nr}_Status`;
            
            if (!row[dataEmissaoKey] || row[dataEmissaoKey] === '') {
                row[vencimentoKey] = null;
                row[statusKey] = '';
            }
        });
        
        // Limpar EPI também
        if (!row.Epi_DataEmissao || row.Epi_DataEmissao === '') {
            row.epiVencimento = null;
            row.EpiStatus = '';
        }
...
app.post('/api/ssma', (req, res) => {
    ...
    // VERIFICAR CPF DUPLICADO - Bloquear CPF duplicado em QUALQUER registro (ativo ou inativo)
    if (cpf && cpf.trim() !== '') {
        // Normalizar CPF: remover espaços e converter para maiúsculo
        const cpfNormalizado = cpf.trim().toUpperCase();
        console.log('🔍 Verificando CPF duplicado em todos os registros...');
        ...
        db.get(checkCpfSql, [cpfNormalizado], (err, cpfRow) => {
            if (err) {
                console.error('Erro ao verificar CPF:', err);
            ...
            if (cpfRow) {
                const statusText = cpfRow.Situacao === 'N' ? 'ATIVO' : 'INATIVO';
                console.log('⚠️ CPF DUPLICADO DETECTADO! ID:', cpfRow.id, 'Nome:', cpfRow.Nome, 'Status:', statusText);
                res.status(409).json({ 
                    error: `CPF já cadastrado para ${cpfRow.Nome} (${cpfRow.Empresa}) - Status: ${statusText}`,
                    duplicateId: cpfRow.id,
                    duplicateStatus: cpfRow.Situacao,
                    duplicateName: cpfRow.Nome,
                    duplicateCompany: cpfRow.Empresa,
                    duplicateType: 'cpf'
                });
                return;
            }
        ...
        db.run(sql, params, function(err) {
            if (err) {
                ...
                if (err.code === 'SQLITE_CONSTRAINT' && err.message.includes('UNIQUE')) {
                    ...
                    // Buscar quem tem o CPF para mostrar na mensagem
                    if (cpf && cpf.trim() !== '') {
                        const cpfNormalizado = cpf.trim().toUpperCase();
                        db.get(
                            `SELECT id, Nome, Empresa, Situacao FROM SSMA WHERE UPPER(TRIM(CPF)) = ? LIMIT 1`,
                            [cpfNormalizado],
                            (errFind, row) => {
                                if (row) {
                                    const statusText = row.Situacao === 'N' ? 'ATIVO' : 'INATIVO';
                                    res.status(409).json({ 
                                        error: `CPF já cadastrado para ${row.Nome} (${row.Empresa}) - Status: ${statusText}`,
                                        duplicateId: row.id,
                                        duplicateStatus: row.Situacao,
                                        duplicateName: row.Nome,
                                        duplicateCompany: row.Empresa,
                                        duplicateType: 'cpf'
                                    });
                                } else {
                                    res.status(409).json({ 
...
app.put('/api/ssma/:id', (req, res) => {
    ...
    let {
        nome, empresa, funcao, celular, cpf, dataEmissao, vencimento, genero,
        nr06_dataEmissao, nr06_vencimento, nr06_status,
        nr10_dataEmissao, nr10_vencimento, nr10_status,
        nr11_dataEmissao, nr11_vencimento, nr11_status,
        nr12_dataEmissao, nr12_vencimento, nr12_status, nr12_ferramenta,
    ...
    // VERIFICAR CPF DUPLICADO - Bloquear CPF duplicado em QUALQUER registro (ativo ou inativo, exceto o próprio)
    if (cpf && cpf.trim() !== '') {
        // Normalizar CPF: remover espaços e converter para maiúsculo
        const cpfNormalizado = cpf.trim().toUpperCase();
        console.log('🔍 Verificando CPF duplicado em todos os registros (excluindo registro atual)...');
        console.log('   CPF normalizado:', cpfNormalizado);
        const checkCpfSql = `
            SELECT id, Nome, Empresa, Situacao FROM SSMA WHERE 
            UPPER(TRIM(CPF)) = ? AND
        ...
        db.get(checkCpfSql, [cpfNormalizado, id], (err, cpfRow) => {
            if (err) {
                console.error('Erro ao verificar CPF:', err);
                res.status(500).json({ error: 'Erro ao verificar CPF' });
                return;
    ...
    db.get('SELECT Funcao, Nome FROM SSMA WHERE id = ?', [id], (err, dadosAtuais) => {
        ...
        // ✅ APLICAR CASCATA: Se Data Emissão vazia → Vencimento, Status e Gênero devem ser NULL
        console.log('🔄 Aplicando regra de cascata para cursos...');
        
        // ASO - Se Data Emissão vazia → zerar Vencimento, Status e Gênero
        if (!dataEmissao || dataEmissao.trim() === '') {
            console.log('   ASO: Data Emissão vazia → zerando Vencimento, Status e Gênero');
            vencimento = null;
            genero = null;
        }
        
        // NR-06
        if (!nr06_dataEmissao || nr06_dataEmissao.trim() === '') {
            console.log('   NR-06: Data Emissão vazia → zerando Vencimento e Status');
            nr06_vencimento = null;
            nr06_status = null;
        }
        
        // NR-10
        if (!nr10_dataEmissao || nr10_dataEmissao.trim() === '') {
            console.log('   NR-10: Data Emissão vazia → zerando Vencimento e Status');
            nr10_vencimento = null;
            nr10_status = null;
        }
        
        // NR-11
        if (!nr11_dataEmissao || nr11_dataEmissao.trim() === '') {
            console.log('   NR-11: Data Emissão vazia → zerando Vencimento e Status');
            nr11_vencimento = null;
            nr11_status = null;
        }
        
        // NR-12
        if (!nr12_dataEmissao || nr12_dataEmissao.trim() === '') {
            console.log('   NR-12: Data Emissão vazia → zerando Vencimento e Status');
            nr12_vencimento = null;
            nr12_status = null;
        }
        
        // NR-17
        if (!nr17_dataEmissao || nr17_dataEmissao.trim() === '') {
            console.log('   NR-17: Data Emissão vazia → zerando Vencimento e Status');
            nr17_vencimento = null;
            nr17_status = null;
        }
        
        // NR-18
        if (!nr18_dataEmissao || nr18_dataEmissao.trim() === '') {
            console.log('   NR-18: Data Emissão vazia → zerando Vencimento e Status');
            nr18_vencimento = null;
            nr18_status = null;
        }
        
        // NR-20
        if (!nr20_dataEmissao || nr20_dataEmissao.trim() === '') {
            console.log('   NR-20: Data Emissão vazia → zerando Vencimento e Status');
            nr20_vencimento = null;
            nr20_status = null;
        }
        
        // NR-33
        if (!nr33_dataEmissao || nr33_dataEmissao.trim() === '') {
            console.log('   NR-33: Data Emissão vazia → zerando Vencimento e Status');
            nr33_vencimento = null;
            nr33_status = null;
        }
        
        // NR-34
        if (!nr34_dataEmissao || nr34_dataEmissao.trim() === '') {
            console.log('   NR-34: Data Emissão vazia → zerando Vencimento e Status');
            nr34_vencimento = null;
            nr34_status = null;
        }
        
        // NR-35
        if (!nr35_dataEmissao || nr35_dataEmissao.trim() === '') {
            console.log('   NR-35: Data Emissão vazia → zerando Vencimento e Status');
            nr35_vencimento = null;
            nr35_status = null;
        }
        
        // EPI
        if (!epi_dataEmissao || epi_dataEmissao.trim() === '') {
            console.log('   EPI: Data Emissão vazia → zerando Vencimento e Status');
            epi_vencimento = null;
            epi_status = null;
        }
        ...
        if (err) {
            ...
            if (err.code === 'SQLITE_CONSTRAINT' && err.message.includes('UNIQUE')) {
                ...
                if (cpf && cpf.trim() !== '') {
                    const cpfNormalizado = cpf.trim().toUpperCase();
                    db.get(
                        `SELECT id, Nome, Empresa, Situacao FROM SSMA WHERE UPPER(TRIM(CPF)) = ? AND id != ? LIMIT 1`,
                        [cpfNormalizado, id],
                        (errFind, row) => {
                            if (row) {
                                const statusText = row.Situacao === 'N' ? 'ATIVO' : 'INATIVO';
    ...
    db.all(sql, params, (err, rows) => {
        ...
        if (rows) {
            rows.sort((a, b) => {
                ...
                if (a.Empresa !== 'HOSS' && b.Empresa === 'HOSS') return 1;
                // Se ambas são HOSS, ordenar por nome
                if (a.Empresa === 'HOSS' && b.Empresa === 'HOSS') return a.Empresa.localeCompare(b.Empresa);
                // Outras empresas: ordenar alfabeticamente
...
app.get('/api/documentacao', (req, res) => {
    db.all('SELECT * FROM DOCUMENTACAO', (err, rows) => {
        if (err) {
            res.status(500).json({ error: err.message });
            return;
        }
...
// Função auxiliar para formatar Celular
function formatarCelular(celular) {
    ...
    if (celularLimpo.length === 11) {
        ...
        return `(${celularLimpo.substring(0, 2)}) ${celularLimpo.substring(2, 6)}-${celularLimpo.substring(6)}`;
    }
    return celular;
}
...
app.get('/api/controle-presenca/funcionarios', (req, res) => {
    ...
    // Buscar todos os funcionários ativos (sem ORDER BY - vamos ordenar customizado)
    const sql = `SELECT id, Nome, Empresa, Funcao, Situacao FROM SSMA WHERE Situacao = 'N'`;
    ...
    db.all(sql, [], (err, rows) => {
        ...
        // Pegar IDs dos funcionários que têm presença registrada no mês atual
        const dadosPresencaMesAtual = presencaMemoria[presencaMesAtual] || {};
        const idsComPresenca = Object.keys(dadosPresencaMesAtual).map(id => parseInt(id));
        ...
        if (idsComPresenca.length > 0) {
            ...
            db.all(sqlInativos, idsComPresenca, (err2, inativos) => {
                if (err2) {
                    console.error('Erro ao buscar inativos:', err2);
                }
                
                // Combinar ativos + inativos com presença
                const todosFuncionarios = [...rows, ...(inativos || [])];
                
                // ⭐ ORDENAÇÃO CUSTOMIZADA: ATIVOS PRIMEIRO, DEPOIS INATIVOS, HOSS SEMPRE EM PRIMEIRO
                todosFuncionarios.sort((a, b) => {
                    const aInativo = a.Situacao === 'S';
                    const bInativo = b.Situacao === 'S';
                    
                    // Ativos sempre antes de inativos
                    if (!aInativo && bInativo) return -1;
                    if (aInativo && !bInativo) return 1;
                    
                    // Dentro do mesmo grupo (ativos ou inativos):
                    // HOSS sempre em primeiro
                    if (a.Empresa === 'HOSS' && b.Empresa !== 'HOSS') return -1;
                    if (a.Empresa !== 'HOSS' && b.Empresa === 'HOSS') return 1;
                    // Se ambas são HOSS, ordenar por nome
                ...
                // Buscar mudanças de função do mês atual (COMENTADO PARA OTIMIZAÇÃO)
                // const sqlMudancas = `SELECT * FROM MUDANCA_FUNCAO_PRESENCA WHERE mesAno = ? ORDER BY funcionarioId, diaInicio`;
                
                // Retornar funcionários sem processar mudanças de função (otimização de performance)
                const funcionariosExpandidos = todosFuncionarios.map(func => ({
                    ...func,
            ...
            // Nenhum inativo com presença, retornar apenas ativos (otimização)
            const funcionariosExpandidos = rows.map(func => ({
                ...func,
                inativado: false,
                funcaoOriginal: func.Funcao,
                diaInicio: 1,
...
// POST - Salvar dados de presença para o histórico
app.post('/api/controle-presenca/salvar-historico', (req, res) => {
    verificarResetMes();
    
    console.log(`📦 Iniciando salvamento de presença para histórico - ${presencaMesAtual}...`);
    
    const mesAno = presencaMesAtual;
    const dadosPresenca = presencaMemoria[mesAno] || {};
    
    // Verificar se há dados para salvar
    if (Object.keys(dadosPresenca).length === 0) {
        return res.json({ success: false, message: 'Nenhum dado de presença para salvar' });
    }
    
    // Buscar funcionários
    db.all(`SELECT id, Nome, Empresa, Funcao, Situacao FROM SSMA`, [], (err, funcionarios) => {
        if (err) {
            console.error('❌ Erro ao buscar funcionários:', err);
            return res.json({ success: false, message: 'Erro ao buscar funcionários' });
        }
        ...
        // Aguardar um pouco para garantir que todos os inserts foram executados
        setTimeout(() => {
            console.log(`✅ Histórico salvo: ${salvos} funcionários com presença registrada`);
            if (erros > 0) {
                console.log(`⚠️ ${erros} erro(s) durante o salvamento`);
            }
            
            res.json({ 
                success: true, 
                message: `Presença salva para o histórico! ${salvos} funcionários registrados`,
                salvos: salvos,
                erros: erros,
                mesAno: mesAno
            });
...
app.post('/api/controle-presenca/ocultar', (req, res) => {
    ...
    if (!funcionariosOcultos[presencaMesAtual]) {
        funcionariosOcultos[presencaMesAtual] = {};
    }
    
    funcionariosOcultos[presencaMesAtual][funcionarioId] = true;
    salvarDadosPresenca();
...
app.post('/api/controle-presenca/mostrar', (req, res) => {
    ...
    if (funcionariosOcultos[presencaMesAtual]) {
        delete funcionariosOcultos[presencaMesAtual][funcionarioId];
        salvarDadosPresenca();
    }
...
// ============ FUNÇÃO AUXILIAR PARA GERAR EXCEL DE PRESENÇA ============
async function gerarExcelPresenca(funcionarios, titulo, incluirInativos = false) {
    const workbook = new ExcelJS.Workbook();
    const sheet = workbook.addWorksheet('Presença');
    ...
    for (const func of funcionariosOrdenados) {
        ...
        for (let dia = 1; dia <= diasNoMes; dia++) {
            ...
            // Se é inativo, aplicar fundo amarelo em TODAS as células
            if (isInativo) {
                row.getCell(3 + dia).fill = {
                    type: 'pattern',
                    pattern: 'solid',
        ...
        // Se é inativo, aplicar fundo amarelo nas colunas de totais também
        if (isInativo) {
            row.getCell(4 + diasNoMes).fill = {
                type: 'pattern',
                pattern: 'solid',
                fgColor: { argb: 'FFFFFF99' }
            };
            row.getCell(5 + diasNoMes).fill = {
                type: 'pattern',
                pattern: 'solid',
                fgColor: { argb: 'FFFFFF99' }
            };
            // Também nas colunas de empresa, nome e função
            row.getCell(1).fill = {
                type: 'pattern',
                pattern: 'solid',
                fgColor: { argb: 'FFFFFF99' }
    ...
    for (const func of funcionarios) {
        ...
        if (!resumoPorFuncao[func.Funcao]) {
            ...
            for (let dia = 1; dia <= diasNoMes; dia++) {
                resumoPorFuncao[func.Funcao][dia] = { P: 0, F: 0, total: 0 };
            }
        }
        ...
        for (let dia = 1; dia <= diasNoMes; dia++) {
            ...
            if (typeof dadosDia === 'object' && dadosDia !== null) {
                valorExibir = dadosDia.isFolga ? '-' : (dadosDia.status || '');
            } else if (typeof dadosDia === 'string') {
                valorExibir = dadosDia;
            }
            
            if (valorExibir === 'P') {
                resumoPorFuncao[func.Funcao][dia].P++;
            } else if (valorExibir === 'F') {
                resumoPorFuncao[func.Funcao][dia].F++;
            }
            resumoPorFuncao[func.Funcao][dia].total++;
        }
    }
    ...
    // Cabeçalho do resumo por função
    ...
    for (const funcao of funcoes) {
        ...
        row.getCell(2 + diasNoMes).alignment = { horizontal: 'center' };
        
        // Coluna de faltas
        let totalFaltas = 0;
        for (let dia = 1; dia <= diasNoMes; dia++) {
            totalFaltas += resumoPorFuncao[funcao][dia].F;
        }
        row.getCell(3 + diasNoMes).value = totalFaltas;
        row.getCell(3 + diasNoMes).font = { bold: true };
        row.getCell(3 + diasNoMes).alignment = { horizontal: 'center' };
        
        // Bordas
        for (let c = 1; c <= 3 + diasNoMes; c++) {
    ...
    rowTotal.getCell(1).font = { bold: true, color: { argb: 'FFFFFFFF' } };
    rowTotal.getCell(1).fill = {
        type: 'pattern',
        pattern: 'solid',
    ...
    for (let dia = 1; dia <= diasNoMes; dia++) {
        let diaTotal = 0;
        for (const funcao of funcoes) {
            diaTotal += resumoPorFuncao[funcao][dia].P;
        }
        rowTotal.getCell(1 + dia).value = diaTotal;
        rowTotal.getCell(1 + dia).font = { bold: true, color: { argb: 'FFFFFFFF' } };
        rowTotal.getCell(1 + dia).fill = {
            type: 'pattern',
            pattern: 'solid',
            fgColor: { argb: 'FF000000' }
        };
        rowTotal.getCell(1 + dia).alignment = { horizontal: 'center' };
        totalGeralP += diaTotal;
    }
    
    rowTotal.getCell(2 + diasNoMes).value = totalGeralP;
    rowTotal.getCell(2 + diasNoMes).font = { bold: true, color: { argb: 'FFFFFFFF' } };
    rowTotal.getCell(2 + diasNoMes).fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'FF000000' }
    };
    rowTotal.getCell(2 + diasNoMes).alignment = { horizontal: 'center' };
    
    let totalGeralF = 0;
    for (const funcao of funcoes) {
        for (let dia = 1; dia <= diasNoMes; dia++) {
            totalGeralF += resumoPorFuncao[funcao][dia].F;
        }
    }
    rowTotal.getCell(3 + diasNoMes).value = totalGeralF;
    rowTotal.getCell(3 + diasNoMes).font = { bold: true, color: { argb: 'FFFFFFFF' } };
    rowTotal.getCell(3 + diasNoMes).fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'FF000000' }
    };
    rowTotal.getCell(3 + diasNoMes).alignment = { horizontal: 'center' };
    
    // Bordas do total
    for (let c = 1; c <= 3 + diasNoMes; c++) {
        rowTotal.getCell(c).border = {
            top: { style: 'thin' },
            left: { style: 'thin' },
            bottom: { style: 'thin' },
            right: { style: 'thin' }
    ...
    for (const func of funcionarios) {
        ...
        if (!resumoPorEmpresa[func.Empresa]) {
            ...
            for (let dia = 1; dia <= diasNoMes; dia++) {
                resumoPorEmpresa[func.Empresa][dia] = { P: 0, F: 0, total: 0 };
            }
        }
        ...
        for (let dia = 1; dia <= diasNoMes; dia++) {
            ...
            if (valorExibir === 'P') {
                resumoPorEmpresa[func.Empresa][dia].P++;
            } else if (valorExibir === 'F') {
                resumoPorEmpresa[func.Empresa][dia].F++;
            }
            resumoPorEmpresa[func.Empresa][dia].total++;
        }
    }
    ...
    // Cabeçalho do resumo por empresa
    ...
    for (const empresa of empresas) {
        ...
        const temAtivos = funcionariosOrdenados.some(f => f.Empresa === empresa && f.Situacao !== 'S');
        const corFundo = temAtivos ? 'FF87CEEB' : 'FFFFFF99'; // Azul claro para ativos, amarelo para inativos
        ...
        let totalFaltas = 0;
        for (let dia = 1; dia <= diasNoMes; dia++) {
            totalFaltas += resumoPorEmpresa[empresa][dia].F;
        }
        row.getCell(3 + diasNoMes).value = totalFaltas;
        row.getCell(3 + diasNoMes).font = { bold: true };
        row.getCell(3 + diasNoMes).alignment = { horizontal: 'center' };
        row.getCell(3 + diasNoMes).fill = {
            type: 'pattern',
            pattern: 'solid',
            fgColor: { argb: corFundo }
        };
    ...
    rowTotalEmpresa.getCell(1).fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'FF000000' }
    };
    
    let totalGeralPEmpresa = 0;
    for (let dia = 1; dia <= diasNoMes; dia++) {
        let diaTotal = 0;
        for (const empresa of empresas) {
            diaTotal += resumoPorEmpresa[empresa][dia].P;
        }
        rowTotalEmpresa.getCell(1 + dia).value = diaTotal;
        rowTotalEmpresa.getCell(1 + dia).font = { bold: true, color: { argb: 'FFFFFFFF' } };
        rowTotalEmpresa.getCell(1 + dia).fill = {
            type: 'pattern',
            pattern: 'solid',
            fgColor: { argb: 'FF000000' }
        };
    ...
    rowTotalEmpresa.getCell(2 + diasNoMes).fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'FF000000' }
    };
    rowTotalEmpresa.getCell(2 + diasNoMes).alignment = { horizontal: 'center' };
    
    let totalGeralFEmpresa = 0;
    for (const empresa of empresas) {
        for (let dia = 1; dia <= diasNoMes; dia++) {
            totalGeralFEmpresa += resumoPorEmpresa[empresa][dia].F;
        }
    }
    ...
    rowTotalEmpresa.getCell(3 + diasNoMes).fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'FF000000' }
    };
    rowTotalEmpresa.getCell(3 + diasNoMes).alignment = { horizontal: 'center' };
    
    // Bordas do total por empresa
    for (let c = 1; c <= 3 + diasNoMes; c++) {
        rowTotalEmpresa.getCell(c).border = {
            top: { style: 'thin' },
            left: { style: 'thin' },
            bottom: { style: 'thin' },
            right: { style: 'thin' }
...
// POST - Exportar presença para Excel (Padrão: Ativos + Inativados com presença)
app.post('/api/controle-presenca/exportar', async (req, res) => {
    ...
    try {
        ...
        // Buscar funcionários ativos + inativados que tiveram presença no mês
        const funcionarios = await new Promise((resolve, reject) => {
            const dadosPresenca = presencaMemoria[presencaMesAtual] || {};
            const idsComPresenca = Object.keys(dadosPresenca).map(id => parseInt(id));
            ...
            if (idsComPresenca.length > 0) {
                ...
                db.all(sql, idsComPresenca, (err, rows) => {
                    if (err) reject(err);
                    else resolve(rows);
                });
            } else {
        ...
        // ⭐ ORDENAÇÃO CUSTOMIZADA: HOSS SEMPRE EM PRIMEIRO (ANTES de gerar o Excel)
        funcionarios.sort((a, b) => {
            // HOSS sempre em primeiro
            if (a.Empresa === 'HOSS' && b.Empresa !== 'HOSS') return -1;
            if (a.Empresa !== 'HOSS' && b.Empresa === 'HOSS') return 1;
        ...
        const buffer = await gerarExcelPresenca(funcionarios, titulo || 'CONTROLE DE PRESENÇA');
...
// POST - Exportar APENAS inativados do mês
app.post('/api/controle-presenca/exportar-inativos', async (req, res) => {
    verificarResetMes();
    
    try {
        const { titulo } = req.body;
        
        // Buscar APENAS funcionários inativos que tiveram presença no mês
        const funcionarios = await new Promise((resolve, reject) => {
            const dadosPresenca = presencaMemoria[presencaMesAtual] || {};
            const idsComPresenca = Object.keys(dadosPresenca).map(id => parseInt(id));
            ...
            const placeholders = idsComPresenca.map(() => '?').join(',');
            const sql = `SELECT id, Nome, Empresa, Funcao, Situacao FROM SSMA WHERE Situacao = 'S' AND id IN (${placeholders})`;
        ...
        const buffer = await gerarExcelPresenca(funcionarios, titulo || 'CONTROLE DE PRESENÇA - INATIVOS');
        ...
        res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        res.setHeader('Content-Disposition', `attachment; filename=Presenca_Inativos_${meses[mes]}_${ano}.xlsx`);
        res.send(buffer);
        
    } catch (error) {
        console.error('Erro ao exportar presença de inativos:', error);
        res.status(500).json({ error: error.message });
    }
});

// POST - Exportar TODOS os funcionários (completo, sem filtros)
app.post('/api/controle-presenca/exportar-completo', async (req, res) => {
    verificarResetMes();
    
    try {
        const { titulo } = req.body;
        
        // Buscar TODOS os funcionários do banco (sem WHERE)
        const funcionarios = await new Promise((resolve, reject) => {
            const sql = `SELECT id, Nome, Empresa, Funcao, Situacao FROM SSMA`;
        ...
        const buffer = await gerarExcelPresenca(funcionarios, titulo || 'CONTROLE DE PRESENÇA - BASE COMPLETA');
        ...
        res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        res.setHeader('Content-Disposition', `attachment; filename=Presenca_Completo_${meses[mes]}_${ano}.xlsx`);
        res.send(buffer);
        
    } catch (error) {
        console.error('Erro ao exportar presença completa:', error);
        res.status(500).json({ error: error.message });
    }
});
...
// ✅ CORRIGIR CASCATA DE CURSOS - Se Data Emissão vazia → Vencimento e Status devem ser NULL
app.post('/api/admin/corrigir-cascata-cursos', (req, res) => {
    console.log('🔧 Iniciando correção de cascata para Data Emissão em todos os cursos...\n');
    
    const cursos = [
        { campo: 'Nr06', dataEmissao: 'Nr06_DataEmissao', vencimento: 'Nr06_Vencimento', status: 'Nr06_Status' },
        { campo: 'Nr10', dataEmissao: 'Nr10_DataEmissao', vencimento: 'Nr10_Vencimento', status: 'Nr10_Status' },
        { campo: 'Nr11', dataEmissao: 'Nr11_DataEmissao', vencimento: 'Nr11_Vencimento', status: 'Nr11_Status' },
        { campo: 'Nr12', dataEmissao: 'Nr12_DataEmissao', vencimento: 'Nr12_Vencimento', status: 'Nr12_Status' },
        { campo: 'Nr17', dataEmissao: 'Nr17_DataEmissao', vencimento: 'Nr17_Vencimento', status: 'Nr17_Status' },
        { campo: 'Nr18', dataEmissao: 'Nr18_DataEmissao', vencimento: 'Nr18_Vencimento', status: 'Nr18_Status' },
        { campo: 'Nr20', dataEmissao: 'Nr20_DataEmissao', vencimento: 'Nr20_Vencimento', status: 'Nr20_Status' },
        { campo: 'Nr33', dataEmissao: 'Nr33_DataEmissao', vencimento: 'Nr33_Vencimento', status: 'Nr33_Status' },
        { campo: 'Nr34', dataEmissao: 'Nr34_DataEmissao', vencimento: 'Nr34_Vencimento', status: 'Nr34_Status' },
        { campo: 'Nr35', dataEmissao: 'Nr35_DataEmissao', vencimento: 'Nr35_Vencimento', status: 'Nr35_Status' },
        { campo: 'EPI', dataEmissao: 'Epi_DataEmissao', vencimento: 'epiVencimento', status: 'EpiStatus' },
        { campo: 'ASO', dataEmissao: 'DataEmissao', vencimento: 'Vencimento', status: null }
    ];
    ...
    db.serialize(() => {
        cursos.forEach(curso => {
            const sql = `
                UPDATE SSMA 
                SET ${curso.vencimento} = NULL${curso.status ? ', ' + curso.status + ' = NULL' : ''}
                WHERE (${curso.dataEmissao} IS NULL OR ${curso.dataEmissao} = '')
                AND (${curso.vencimento} IS NOT NULL OR ${curso.status ? curso.status + ' IS NOT NULL' : 'FALSE'})
            `;
            
            db.run(sql, function(err) {
                if (err) {
                    console.error(`❌ Erro ao corrigir ${curso.campo}:`, err.message);
                } else {
                    if (this.changes > 0) {
                        console.log(`✅ ${curso.campo}: ${this.changes} registros corrigidos`);
                        totalCorrigidos += this.changes;
                    } else {
                        console.log(`✅ ${curso.campo}: nenhum registro para corrigir`);
                    }
                }
                
                processados++;
                
                // Se processou todos os cursos, enviar resposta
                if (processados === cursos.length) {
                    console.log(`\n✨ Correção concluída! Total de registros corrigidos: ${totalCorrigidos}`);
                    res.json({ 
                        success: true, 
                        message: `Cascata corrigida com sucesso em ${totalCorrigidos} registros`,
...
app.get('/api/empresas', (req, res) => {
    ...
    db.all('SELECT DISTINCT Empresa FROM FORNECEDOR WHERE Situacao = "S" AND Empresa IS NOT NULL AND Empresa != ""', (err, rows) => {
        if (err) {
            res.status(500).json({ error: err.message });
            return;
        }
