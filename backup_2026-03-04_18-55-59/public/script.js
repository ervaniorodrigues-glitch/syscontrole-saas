...
class SysControleWeb {
        ...
        try {
            ...
            for (const func of funcionarios) {
                ...
                for (let dia = 1; dia <= diasNoMes; dia++) {
                    ...
                    // ⭐ CORREÇÃO: Aplicar azul para TODOS os campos (vazios ou não) se for fim de semana/feriado
                    const statusClass = status ? `status-${status.toLowerCase()}` : (isFolgaSalva || isFimDeSemana ? 'dia-folga-input' : '');
                    const inputClass = isFimDeSemana || isFolgaSalva ? 'dia-folga-input' : '';
                    ...
                    bodyHtml += `data-func="${func.id}" data-dia="${dia}" data-nome="${func.Nome || ''}" data-funcao="${func.Funcao || ''}" data-empresa="${func.Empresa || ''}" data-folga="${isFolgaSalva || isFimDeSemana}" ${comentarioData} ${comentarioTitle} `;
                    ...
                    if (!isComum && !isIntermediario && !isInativado && !diaForaPeriodo) {
                        ...
                        bodyHtml += `onblur="syscontrole.salvarPresencaIndividual(this)" `;
                        bodyHtml += `onkeypress="syscontrole.handlePresencaKeypress(event, this)" `;
    ...
    formatarPresenca(input) {
        ...
        if (valor === '.' || valor === '-') {
            ...
            celulasParaFormatar.forEach(inp => {
                // LIMPAR O PONTO - não salvar no banco
                inp.value = '';
    ...
    async salvarPresencaIndividual(input) {
        ...
        // ⭐ CRÍTICO: Se o valor é um ponto, NÃO SALVAR - apenas formatar
        if (valor === '.' || valor === '-') {
            // Não fazer nada - apenas aplicar formatação no frontend
            input.value = '';
            input.dataset.folga = 'true';
            input.className = 'presenca-input dia-folga-input';
            return; // NÃO SALVAR NO BANCO
        }
